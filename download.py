...
def download_pages():
    list_dir = os.listdir('./j-archive')
    list_dir = [int(f.replace('.html', '')) for f in list_dir]   # Convert to lower case
    list_dir = sorted(list_dir)

    page = list_dir[len(list_dir) - 1] + 1
    ...
    with futures.ThreadPoolExecutor(max_workers=NUM_THREADS) as executor:
        ...
        while True:
            page_file = '{page}.html'
            if not os.path.exists(os.path.join(archive_folder, page_file)):
                l = []
